...
def modify_res_data(res_data, records, timetable):
    for record in records:
        room_status = res_data.get(record.room_name)
        if room_status is None:  # 说明所申请到的教室不在教室信息表中
            continue
        else:
            modify_room_status(room_status, record, timetable)
...
def stu_room_use_info():
    ...
    # 检查json数据是否有缺少
    try:
        common.check_key(json_data, 'date', 'building', 'floor')
    except KeyError:
        return jsonify(code=-101, data=None)
    ...
    rooms = Room.query.filter(and_(Room.building == building, Room.floor == floor))
    ...
    records = Apply.query.filter(and_(Apply.check_status == "审核通过", Apply.use_date == json_data.get('date'),
                                      Apply.building == building, Apply.floor == floor)).all()
